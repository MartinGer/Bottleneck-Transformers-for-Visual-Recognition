...
def run_training(ds, lr, epochs, output, num_heads, img_size, name):      
    ...
    learn = Learner(ds, model, opt_func=Adam, metrics=accuracy, 
                    cbs=[
                        CSVLogger(fname = './logs/attention/imagenette{}_BoTNet_{}.csv'.format(TIME, name), append=True),
                        SaveModelCallback(monitor='accuracy', fname= output + 'imagenette{}_BotNet_{}'.format(TIME, name))
                    ]
                   )
